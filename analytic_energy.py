...
def getAnalyticEnergy(k_x, k_y, phi_x, phi_y, w_0, mu, Delta, B_x, B_y, Lambda):
    A = -2*w_0 * (np.cos(k_x)*np.cos(phi_x) + np.cos(k_y)*np.cos(phi_y))
    B = -2*w_0 * (np.sin(k_x)*np.sin(phi_x) + np.sin(k_y)*np.sin(phi_y))
    C = -B_y + Lambda * np.cos(k_x)*np.sin(phi_x)
    D = -B_x - Lambda * np.cos(k_y)*np.sin(phi_y)
    E = Lambda * np.sin(k_x)*np.cos(phi_x)
    F = -Lambda * np.sin(k_y)*np.cos(phi_y) 
...
def getAnalyticEnergies(k_x_values, k_y_values, phi_x_values, phi_y_values, w_0, mu, Delta, B_x, B_y, Lambda):
        ...
        for j, k_y in enumerate(k_y_values):
            for k, phi_x in enumerate(phi_x_values):
                for l, phi_y in enumerate(phi_y_values):
                    A = -2*w_0 * (np.cos(k_x)*np.cos(phi_x) + np.cos(k_y)*np.cos(phi_y))
                    B = 2*w_0 * (np.sin(k_x)*np.sin(phi_x) + np.sin(k_y)*np.sin(phi_y))
                    C = -B_y + 2*Lambda * np.cos(k_x)*np.sin(phi_x)
                    D = -B_x - 2*Lambda * np.cos(k_y)*np.sin(phi_y)
                    E = 2*Lambda * np.sin(k_x)*np.cos(phi_x)
                    F = -2*Lambda * np.sin(k_y)*np.cos(phi_y)
